...
for _ in {1..30}; do
  ...
  if BRIDGE_STATUS_JSON="$current" node -e 'const s=JSON.parse(process.env.BRIDGE_STATUS_JSON || "{}"); process.exit(s.process_running === true ? 1 : 0)'; then
    remaining_lease="$(find "$ARTIFACT_ROOT/runtime" -maxdepth 1 -type d -name 'device-*.lock' -print -quit 2>/dev/null || true)"
    if [[ -n "$remaining_lease" ]]; then
      printf 'ERROR: Runtime stopped but a device lease remains; cleanup is not proven.\n' >&2
      exit 2
    fi
